...
# ---------------------------------------------------------------------------
# Node-budget entry point
# ---------------------------------------------------------------------------

def fit_curve_max_nodes(points, max_nodes, tol=1.0e-3, iters=48):
    """Fit cubic Bezier segments using AT MOST `max_nodes` on-curve nodes.

    `max_nodes` counts anchor points (on-curve), so the result has at most
    `max_nodes - 1` cubic segments. The first and last nodes are FIXED to the
    first and last input points exactly.

    Strategy: Schneider's fitter takes a max_error and splits as needed; the
    segment count falls as the error rises. We search for the smallest error
    that still meets the node budget (i.e. use as much detail as allowed), then
    pin the endpoints.
    """
    points = np.asarray(points, dtype=float)
    n = points.shape[0]
    ...
    max_segs = max_nodes - 1

    # Finest possible fit; if it already fits the budget, we are done.
    best = fit_curve(points, 1.0e-9)
    if len(best) <= max_segs:
        return _pin_ends(best, points)

    # Grow an upper error bound until the budget is satisfied.
    span = points.max(axis=0) - points.min(axis=0)
    hi = max(float(np.hypot(span[0], span[1])), 1.0)
    while len(fit_curve(points, hi)) > max_segs:
        hi *= 2.0
    best = fit_curve(points, hi)

    # Binary-search the smallest error that still satisfies the budget.
    lo = 0.0
    for _ in range(iters):
        mid = 0.5 * (lo + hi)
        c = fit_curve(points, mid)
        if len(c) <= max_segs:
            best = c
            hi = mid
        else:
...
def _pin_ends(curves, points):
    """Force the very first/last anchors to the exact endpoint samples."""
    if curves:
        curves[0][0] = np.asarray(points[0], dtype=float)
        curves[-1][3] = np.asarray(points[-1], dtype=float)
    return curves
