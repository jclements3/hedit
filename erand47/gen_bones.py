...
# ===================== 8. DISC / PRONG SIZING (real, per string) ===============
# Pulled from the sizing analysis so the script carries the numbers, not prose.
LB2N=4.448222
T_N    = TENSION_LBF*LB2N
WRAP   = math.radians(15.0)                 # engaged wrap at the prong
F_side = 2*T_N*np.sin(WRAP/2)               # lateral load per prong contact (N)
ELL    = PLATE_GAP_MM/2                      # prong reach (depth) = 6.5 mm
SIG    = 300.0; AIR = 0.8                    # bending allow.; disengaged air/side
d_prong= np.ceil((32*F_side*ELL/(math.pi*SIG))**(1/3)*10)/10   # round Ø up to 0.1
RP     = d_prong/2                           # prong radius (per string)
PC     = RP + DIA/2 + AIR                    # axle -> prong centre (straddles string w/ AIR/side)
LH     = PC + RP                             # disc half-length (tip)
WW     = 2*(RP+0.3)                          # disc width

# ===================== 8b. BONE PATHS (axle offset by real pc) =================
def _sname(i): return "CDEFGAB"[i%7].lower()+str(1+i//7)
STRING_NAME=[_sname(i) for i in range(N_STRINGS)]; CLASS_OF=["CDEFGAB"[i%7] for i in range(N_STRINGS)]
ENGAGE_DEG=55.0; CRANK_R_MM=8.0; LEVER_HALF_DEG=30.0; LEVER_SIDE=+1
BC_PIVOT_OFF=np.array([-30.0,-8.0]); BC_IN_ARM=np.array([-5.0,-28.0]); BC_OUT_ARM=np.array([24.0,8.0]); CTRL_ROD_LEN=130.0
def _rot(v,deg): a=math.radians(deg); c,s=math.cos(a),math.sin(a); return np.array([v[0]*c-v[1]*s, v[0]*s+v[1]*c])
def _disc_points(node,s_hat,pc_i):
    p=np.array([-s_hat[1],s_hat[0]]); d1=_rot(p,ENGAGE_DEG); d2=_rot(-p,ENGAGE_DEG)
    lo=d1 if d1[1]<d2[1] else d2; axle=node-pc_i*lo
    perp=LEVER_SIDE*p; ti=axle+CRANK_R_MM*_rot(perp,+LEVER_HALF_DEG); to=axle+CRANK_R_MM*_rot(perp,-LEVER_HALF_DEG)
    return axle,ti,to

# ===================== 8c. STATE DIAGRAM (to scale, real numbers) ===============
def draw_state_scale(i, fname):
    rp,pc,Lh,ww,od = RP[i],PC[i],LH[i],WW[i],DIA[i]
    zsep=(F_SHP-F_NAT)*L[i]
    sc=24; W=940; H=620; col=[200,470,740]; yT=110; yNAT=200; ySHP=330; yLIVE=560
    o=[f'<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" viewBox="0 0 {W} {H}" font-family="Georgia,serif">',
       '''<defs><linearGradient id="cap" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#e6eaef"/><stop offset="1" stop-color="#737983"/></linearGradient>
       ...
       f'<rect width="{W}" height="{H}" fill="#13110d"/>',
       f'<text x="{W/2}" y="36" fill="#eef0f4" font-size="15" text-anchor="middle">{STRING_NAME[i].upper()} disc/prong state — to scale (Ø{od:.2f}, prong Ø{2*rp:.1f}, pc {pc:.2f}, disc {2*Lh:.1f}×{ww:.1f} mm)</text>']
    def disc(cx,cy,engaged,color):
        ang=-ENGAGE_DEG if engaged else 0.0
        g=[f'<g transform="rotate({ang} {cx} {cy})">',
           f'<rect x="{cx-Lh*sc:.1f}" y="{cy-ww/2*sc:.1f}" width="{2*Lh*sc:.1f}" height="{ww*sc:.1f}" rx="{ww/2*sc:.1f}" fill="url(#cap)" stroke="{color}" stroke-width="2"/>']
        for sgn in (1,-1): g.append(f'<circle cx="{cx+sgn*pc*sc:.1f}" cy="{cy}" r="{rp*sc:.1f}" fill="url(#pin)" stroke="#eef0f4" stroke-width="0.8"/>')
        g.append(f'<circle cx="{cx}" cy="{cy}" r="2.4" fill="#23262b" stroke="#cfd4da" stroke-width="0.7"/></g>')
        return "".join(g)
    for c,(name,act) in enumerate([('flat',None),('natural','n'),('sharp','s')]):
        cx=col[c]; o.append(f'<text x="{cx}" y="70" fill="#eef0f4" font-size="14" text-anchor="middle">pedal in {name}</text>')
        node_y = yT if act is None else (yNAT if act=='n' else ySHP)
        o.append(f'<line x1="{cx}" y1="{yT}" x2="{cx}" y2="{node_y}" stroke="#5a5440" stroke-width="{max(od*sc,2):.1f}"/>')
        o.append(f'<line x1="{cx}" y1="{node_y}" x2="{cx}" y2="{yLIVE}" stroke="#f0c46a" stroke-width="{max(od*sc,2):.1f}"/>')
        o.append(disc(cx,yNAT, act in ('n','s'), '#c0a0ff'))
        o.append(disc(cx,ySHP, act=='s', '#ff8c5a'))
        if act: 
            ny=yNAT if act=='n' else ySHP
            o.append(f'<circle cx="{cx}" cy="{ny}" r="3" fill="#9ad8b0"/><text x="{cx-Lh*sc-6:.0f}" y="{ny+4}" fill="#9ad8b0" font-size="9" text-anchor="end">node</text>')
    o.append(f'<text x="200" y="{yNAT-Lh*sc-8:.0f}" fill="#c0a0ff" font-size="9" text-anchor="middle">natural disc</text>')
    o.append(f'<text x="200" y="{ySHP+ww/2*sc+16:.0f}" fill="#ff8c5a" font-size="9" text-anchor="middle">sharp disc</text>')
    # scale bar 5 mm
    o.append(f'<line x1="60" y1="590" x2="{60+5*sc}" y2="590" stroke="#aeb3bd" stroke-width="2"/><text x="{60+2.5*sc}" y="584" fill="#aeb3bd" font-size="10" text-anchor="middle">5 mm</text>')
    o.append(f'<text x="600" y="592" fill="#8a8f98" font-size="10">flat: prongs straddle string, {AIR:.1f} mm air/side · engage CCW · pins seated flush in the capsule tips</text>')
    o.append('</svg>'); open(fname,"w").write("\n".join(o))

draw_state_scale(20,'disc_state_scale.svg')   # B3
print("\n--- real disc/prong (sample strings) ---")
print("note   Ø     prongØ  pc     disc L×W (mm)")
for i in [0,9,20,29,42,46]:
    print(f"{STRING_NAME[i].upper():4s} {DIA[i]:5.2f}  {2*RP[i]:5.2f}  {PC[i]:5.2f}  {2*LH[i]:5.2f}×{WW[i]:.2f}")
